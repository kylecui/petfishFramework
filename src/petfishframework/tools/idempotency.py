...
class IdempotencyStore:
    ...
    Thread-safe: ``_store`` is protected by ``threading.Lock``.
    """
    ...
    def has(self, key: str) -> bool:
        ...
        with self._lock:
            return self._get_unsafe(key) is not None
    ...
    def _get_unsafe(self, key: str) -> ToolResult | None:
        """Internal get without locking; caller must hold ``_lock``."""
        if key not in self._store:
            return None
